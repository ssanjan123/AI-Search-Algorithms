...
def astar_search(problem, h=None, display=False):
    ...
    n = best_first_graph_search(problem, lambda n: n.path_cost + h(n), display)
    return len(n.solution())
...
# Makes a Stage Puzzle with random initatial State
def make_rand_StagePuzzle():
    initialState = list(range(0,10))
    random.shuffle(initialState)
    initialState = tuple(initialState)
    randStageP = StagePuzzle(initial=initialState,)
    while(randStageP.check_solvability(initialState) == False):
        initialState = list(range(0,10))
        random.shuffle(initialState)
        initialState = tuple(initialState)
        setattr(randStageP,'initial',initialState)
    return randStageP 
    



# Takes an initial stage as a hard code and converts it into a stage puzzle
def make_hardCoded_StagePuzzle(initialS):
    return StagePuzzle(initial=tuple(initialS))




# Displays the stage Puzzle in its block formation
def display(state):
    if state[0] == 0:
        print(' ','*',state[1])
    elif state[1] == 0:
...
#Test each Heuristic with Hard coded initial state to take less time
def testMisplaced():
    i1 = (4,9,3,6,0,2,7,8,5,1) 
    i2 = (4,5,8,3,6,9,1,2,7,0)
    i3 = (1,7,5,4,8,2,3,9,0,6)
    i4 = (8,9,7,1,3,6,4,5,0,2)
    i5 = (1,6,2,4,8,9,0,3,7,5)
    i6 = (1,4,9,5,7,6,3,2,0,8)
    i7 = (6,4,3,1,8,9,7,2,5,0)
    i8 = (6,4,3,8,9,0,2,7,1,5)
    i = (i1,i2,i3,i4,i5,i6,i7,i8)
    
    for j in i:
        display(j)
        print()
        print('Using the above state as our StagePuzzle, we have the following information when running astar_search')
        t = make_hardCoded_StagePuzzle(j)
        AstarRunningData(t,t.h)
...
def testManhattan():
    ...
    for j in i:
        ...
        print('Using the above state as our StagePuzzle, we have the following information when running astar_search')
        t = make_hardCoded_StagePuzzle(j)
        AstarRunningData(t,t.h1)

def testMaxofMisplacedManhattan():
    i1 = (4,9,3,6,0,2,7,8,5,1) 
    i2 = (4,5,8,3,6,9,1,2,7,0)
    i3 = (1,7,5,4,8,2,3,9,0,6)
    i4 = (8,9,7,1,3,6,4,5,0,2)
    ...
    for j in i:
        ...
        print('Using the above state as our StagePuzzle, we have the following information when running astar_search')
        t = make_hardCoded_StagePuzzle(j)
        AstarRunningData(t,t.h2)        

###################################################################################################
#Test all Herestics with 8 Random StagePuzzles
def testALLRandom():
    i1 = make_rand_StagePuzzle() 
    i2 = make_rand_StagePuzzle()
    i3 = make_rand_StagePuzzle()
    i4 = make_rand_StagePuzzle()
    i5 = make_rand_StagePuzzle()
    i6 = make_rand_StagePuzzle()
    i7 = make_rand_StagePuzzle()
    i8 = make_rand_StagePuzzle()
    i = (i1,i2,i3,i4,i5,i6,i7,i8)
    
    for j in i:
        print('Using', j.initial, 'as our initial state, we have the following information when running astar_search using Misplaced Heuristic')
        AstarRunningData(j,j.h)
        
        print('Using', j.initial, 'as our initial state, we have the following information when running astar_search using Manhattan Heuristic')
        AstarRunningData(j,j.h1)
        
        print('Using', j.initial, 'as our initial state, we have the following information when running astar_search using max of Misplaced and Manhattan Heuristic')
        AstarRunningData(j,j.h2)

## Test DATA retrieval

# Takes a Problem and a heuristic and calculates the total running time,
# Solution Length and the number of Node expanded 
# using a*Search with a given h(n)
def AstarRunningData(problem,h):
    start_time = time.time()
    SolutionLength = astar_search(problem,h,True)
    elapsed_time = time.time() - start_time
    print(f'elapsed time (in seconds): {elapsed_time}s')
    print('Solution length is',SolutionLength)


class StagePuzzle(Problem):
    """ The problem of sliding tiles numbered from 1 to 9 on a board with 10 squares, where one of the
    squares is a blank. A state is represented as a tuple of length 10, where  element at
    index i represents the tile number  at index i (0 if it's an empty square) """

    def __init__(self, initial, goal=(1, 2, 3, 4, 5, 6, 7, 8, 9, 0)):
    ...
    def actions(self, state):
        ...
        if index_blank_square in [0, 1, 2, 5]:
            possible_actions.remove('UP')
        if index_blank_square in [1, 5, 9]:
    ...
    def find_blank_square_row(self, state):
    ...
    def result(self, state, action):
        ...
        row = self.find_blank_square_row(state)
        up = 0
        if row == 1:
            up = -3
        elif row == 2:
    ...
    def check_solvability(self, state):
        ...
        row = self.find_blank_square_row(state)
        desired_inversion =  1 if row < 2 else 0
        inversion = 0
        ...
        return inversion % 2 == desired_inversion
    ...
    ## Xaxis and Yaxis is defined as the horizontal and vertical position of a puzzle block which is derived from the respective indices
        xaxisB = 0 #B here is for self
        yaxisB = 0
        xaxisG = 0 #G is the goal state
        yaxisG = 0
        sum = 0
        for b, g in ((node.state.index(i), self.goal.index(i)) for i in range(0,10)):
            xaxisB = b-2
            xaxisG = g-2
            yaxisB = (b+2)//4
            yaxisG = (g+2)//4
            ...
            if xaxisG < 0:
                ...
                xaxisG = xaxisG%4
            sum += (abs(xaxisB-xaxisG) + abs(yaxisB-yaxisG))
        return sum
    
    def h2(self, node):
        """ Return the heuristic value for a given state. 
        h(n) = max(Misplaced, Manhattan) """

        return max(self.h(node),self.h1(node)) # returns the max of h and h1
